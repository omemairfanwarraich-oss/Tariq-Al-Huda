async function loadRecentPdfs() {
    ...
    track.innerHTML = pdfs.length ? pdfs.map((pdf, index) => `<a class="recent-pdf-tile" href="viewer.html?id=${encodeURIComponent(pdf.id)}"><span class="recent-pdf-index">0${index + 1}</span><span class="pdf-badge">PDF · ID ${escapeRecent(pdf.id)}</span><strong>${escapeRecent(pdf.title)}</strong><small>${escapeRecent(pdf.description)}</small><span class="tile-arrow">Open note <b>→</b></span></a>`).join('') : '<p class="empty-state" style="color: var(--gold-muted);">Open a note and it will appear here.</p>';
}
